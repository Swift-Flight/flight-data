...
/// every consumer build; pulling in swift-crypto (BoringSSL) there would add minutes of
/// host-build time for no benefit. Correctness is pinned by NIST test vectors in the
/// test suite.
public enum SHA256 {
    ...
    public static func digest(_ input: [UInt8]) -> [UInt8] {
        ...
        // Padding: append 0x80, zero-fill to 56 mod 64, then the 64-bit big-endian bit length.
        var message = input
