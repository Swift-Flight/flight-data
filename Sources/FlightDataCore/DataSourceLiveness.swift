...
/// A datasource's liveness probe as a component (§5) — the store-agnostic surface
...
/// *health* (§5 — did the pool's service start and stay up) is tracked by
...
public struct DataSourceLiveness: Sendable {
    ...
    /// introspection (Flight Core §6) — no store-package knowledge required.
