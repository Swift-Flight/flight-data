/// A pooled source of store connections (§2). One per configured store —
/// registered as a singleton in the Container; its long-running work is
/// handed to the ServiceGroup via `FlightModule.service` (Flight Core §7).
///
...
/// channel, and above all not to add a query method (§1: Flight Data Core
/// must never define a universal query abstraction).
///
/// ## The checkout/release pair (design delta D1)
///
/// The design doc presents `withConnection` as the whole contract. Scope-bound
/// checkout (§3) forced one deliberate extension: component factories are
/// *synchronous* (Flight Core §2.1), so the scoped `ScopedConnection` component
