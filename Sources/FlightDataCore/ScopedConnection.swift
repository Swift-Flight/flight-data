...
/// One scope's checked-out connection (§3): the component `register(dataSource:)`
/// registers as `.scoped`.
///
...
/// resolution in that scope — which is the whole §3 property: Flight Web ties
...
///
/// The design doc says "register its Connection type as `.scoped`". Core's
/// `Scope` has no close hooks — closing a scope simply drops its instances,
/// making them "eligible for cleanup" (Flight Core §3). A raw connection
...
public final class ScopedConnection<Source: DataSource>: Sendable {
    /// The name this lease's datasource was registered under (§4).
