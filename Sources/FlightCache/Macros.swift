/// The declarative caching annotations (design §3) — body macros (SE-0415),
...
///   `@Transactional` do not compose on a single method (§3.1);
...
/// under `namespace` + the key-contributing arguments (§4), return it.
/// Concurrent same-key callers coalesce (§8).
///
/// `ttl: nil` defers to the §6 policy: `cache.namespaces.<namespace>`, then
...
/// `allEntries: true`. The flag is explicit (§3): `allEntries: false` with
...
/// new-value parameter — it is not part of the entry's identity (§4).
@attached(body)
